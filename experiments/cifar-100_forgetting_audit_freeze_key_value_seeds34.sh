#!/usr/bin/env bash
# Extend the Key+Value joint freeze with seeds 3 and 4.
set -euo pipefail
...
OUTDIR=${OUTDIR:-${ROOT}/forgetting-audit-freeze-key-value-seeds34}
...
GPUID=${GPUID:-3} OVERWRITE=${OVERWRITE:-1} \
  bash experiments/cifar-100_forgetting_audit_freeze_key_value.sh
