#!/usr/bin/env bash
# Intervention: freeze all prompt experts from Task 2 onward.
# Example: GPUID=1 bash experiments/cifar-100_forgetting_audit_freeze_prompt.sh

set -euo pipefail
DATASET=${DATASET:-cifar-100}
CONFIG=${CONFIG:-configs/cifar-100_prompt_smope.yaml}
OUTDIR=${OUTDIR:-outputs/${DATASET}/10-task/forgetting-audit-freeze-prompt}
GPUID=${GPUID:-1}
REPEAT=${REPEAT:-3}
...
python -u run.py --config "${CONFIG}" \
  ...
  --audit_freeze_component prompt --audit_freeze_from_task 2 \
